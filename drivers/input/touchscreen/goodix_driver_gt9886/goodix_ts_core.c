...
static ssize_t fp_state_show(struct device *dev, struct device_attribute *attr,
			     char *buf)
{
	struct goodix_ts_core *core_data = dev_get_drvdata(dev);
	struct goodix_touch_data *touch_data =
	...
	return snprintf(buf, PAGE_SIZE, "%d,%d,%d\n", touch_data->coords[0].x,
			touch_data->coords[0].y, core_data->fod_pressed);
}
...
int goodix_ts_input_dev_config(struct goodix_ts_core *core_data)
{
	...
	input_mt_init_slots(input_dev, GOODIX_MAX_TOUCH * 2 + 1,
			    INPUT_MT_DIRECT);
...
static int goodix_ts_probe(struct platform_device *pdev)
{
	...
	if (core_data->gtp_tp_class == NULL)
		core_data->gtp_tp_class = class_create("touch");
